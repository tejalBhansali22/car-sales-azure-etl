...
df_src = spark.sql('''
SELECT DISTINCT(Dealer_ID) as Dealer_ID, DealerName FROM parquet.`abfss://silver@cartejaldatalake.dfs.core.windows.net/carsales` ''')
...
df_filter = df_src.join(df_sink, df_src.Dealer_ID == df_sink.Dealer_ID, how='left').select(df_src['Dealer_ID'], df_src['DealerName'], df_sink['dim_dealer_key'])
df_filter.display()
...
if spark.catalog.tableExists('cars_catalog.gold.dim_dealer'):
    delta_table = DeltaTable.forPath(spark, 'abfss://gold@cartejaldatalake.dfs.core.windows.net/dim_dealer')
    delta_table.alias('trg').merge(df_final.alias('src'), 'trg.dim_dealer_key = src.dim_dealer_key').whenMatchedUpdateAll().whenNotMatchedInsertAll().execute()
    
# initial run
else: 
    df_final.write.format('delta').mode('overwrite').option('path','abfss://gold@cartejaldatalake.dfs.core.windows.net/dim_dealer').saveAsTable('cars_catalog.gold.dim_dealer')
