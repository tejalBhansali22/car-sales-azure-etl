...
df_src = spark.sql('''
SELECT DISTINCT(Branch_ID) as Branch_ID, BranchName FROM parquet.`abfss://silver@cartejaldatalake.dfs.core.windows.net/carsales` ''')
...
df_filter = df_src.join(df_sink, df_src.Branch_ID == df_sink.Branch_ID, how='left').select(df_src['Branch_ID'], df_src['BranchName'], df_sink['dim_branch_key'])
df_filter.display()
...
if spark.catalog.tableExists('cars_catalog.gold.dim_branch'):
    delta_table = DeltaTable.forPath(spark, 'abfss://gold@cartejaldatalake.dfs.core.windows.net/dim_branch')
    delta_table.alias('trg').merge(df_final.alias('src'), 'trg.dim_branch_key = src.dim_branch_key').whenMatchedUpdateAll().whenNotMatchedInsertAll().execute()
    
# initial run
else: 
    df_final.write.format('delta').mode('overwrite').option('path','abfss://gold@cartejaldatalake.dfs.core.windows.net/dim_branch').saveAsTable('cars_catalog.gold.dim_branch')
